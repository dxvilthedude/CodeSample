...
using UnityEngine;
[Serializable]
public class ItemContainer :  IItemCoitainer
{
    ...
    public ItemSlot AddItemAt(ItemSlot itemSlot, int slotIndex)
    {
        if (slotIndex < 0 || slotIndex > itemSlots.Length - 1) { return itemSlot; }

        if (itemSlots[slotIndex].item == itemSlot.item)
            return itemSlot;

        itemSlots[slotIndex].item = itemSlot.item;
        itemSlots[slotIndex].quantity = 1;
